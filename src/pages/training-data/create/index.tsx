...
import { createTrainingData } from 'apiSdk/training-data';
...
function TrainingDataCreatePage() {
  ...
  const handleSubmit = async (values: TrainingDataInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createTrainingData(values);
      resetForm();
  ...
  const formik = useFormik<TrainingDataInterface>({
    initialValues: {
      data: '',
      neural_network_id: (router.query.neural_network_id as string) ?? null,
    },
